---
title: "E-values for Adaptive Clinical Trials: A Practical Methodology for Anytime-Valid Monitoring and Adaptation"
author:
...
Adaptive clinical trials increasingly rely on interim analyses, flexible stopping, and data-dependent design modifications. These features can improve ethics and efficiency but complicate statistical guarantees when fixed-horizon test statistics are repeatedly inspected or reused after adaptations. E-values and e-processes provide a practical alternative: they yield anytime-valid tests and confidence sequences that remain valid under optional stopping and optional continuation without requiring a prespecified monitoring schedule.
...
A numerical study compares five monitoring rules in a two-arm binary-endpoint trial: naive repeated $p$-values, a simulation-calibrated O'Brien--Fleming-like group sequential boundary, a betting-martingale e-process, a naive posterior-probability threshold, and a simulation-calibrated Bayesian posterior threshold. The naive methods substantially inflate Type I error under frequent interim looks (approximately 15\% for naive repeated $p$-values and 13\% for a naive posterior threshold at 0.975 with 20 looks). Among the error-controlling methods, the calibrated group sequential rule achieves the highest power (86.1%), the calibrated Bayesian rule is more conservative (68.8%), and the e-value rule provides robust anytime-valid control (Type I error 1.2%) with moderate power (72.3%) that is sensitive to the betting fraction.

Extended simulations examine the effect of monitoring schedule (fixed, irregular, and continuous looks), sensitivity across a grid of design parameters (base rates, effect sizes, and look frequencies), futility monitoring via both confidence sequences and reciprocal e-processes, and a RECOVERY-like large-trial scenario. These reveal that the power gap between group sequential and e-value methods reverses under continuous monitoring and varies widely with design parameters. We discuss the role of composite nulls and nuisance parameters, and hybrid strategies for combining e-values with established methods in confirmatory and platform trial settings. The methodology is implemented in the open-source R package `evalinger`, accompanied by an interactive web application for trial design exploration. Together, the paper, package, and application form a practical toolkit for deploying e-value monitoring in an emerging regulatory landscape that includes the January 2026 FDA draft guidance on Bayesian methodology for drugs and biologics.

Keywords: adaptive clinical trials; interim monitoring; optional stopping; e-values; e-processes; group sequential methods; alpha-spending; Bayesian adaptive designs; safe testing; betting martingale; R package.
...
Group sequential tests typically assume that a sequence of test statistics at interim looks follows a multivariate normal distribution with a known correlation structure driven by information fractions. Canonical boundaries include the Pocock and O'Brien--Fleming families. Alpha-spending functions generalize fixed-schedule boundaries by allocating cumulative Type I error as a function of information time, allowing flexible look times while maintaining the overall error budget [@LanDeMets1983]. These methods are widely accepted in confirmatory trials and are supported by established regulatory expectations.


## E-values and e-processes: a mini-tutorial

This section provides a self-contained introduction to e-values and e-processes for readers coming from a clinical trials background. For comprehensive treatments, see @RamdasGrunwaldVovkShafer2023SAVI, @VovkWang2021Evalues, and @GrunwaldDeHeideKoolen2023SafeTesting.
...
To be clear, $p$-values *can* also be exactly valid in finite samples for certain tests---permutation tests and exact binomial tests are classical examples. But achieving exact validity with $p$-values requires choosing from a restricted class of tests and often sacrifices power or flexibility. E-values obtain finite-sample validity "for free" from their definition: any nonnegative statistic with expected value at most 1 under the null is immediately a valid e-value. In the *sequential* setting, the advantage is even starker: e-values, via Ville's inequality for nonnegative supermartingales, give anytime-valid guarantees at every stopping time simultaneously, while $p$-values generally require either a fixed sample size or corrections such as alpha-spending that must be pre-specified [@LanDeMets1983; @JennisonTurnbull2000].
...
The most intuitive construction of an e-value for a clinical trial uses the **testing by betting** framework [@Shafer2021TestingByBetting; @ShaferVovk2019]. Imagine you start with \$1 and repeatedly bet against the null hypothesis as patients are enrolled:
...
Formally, for a trial with 1:1 randomization and binary outcomes, define $D_i = X_i^T - X_i^C$ for the $i$-th patient pair. Under $H_0: p_T = p_C$, we have $\mathbb{E}[D_i] = 0$ regardless of the common response rate. The bettor's wealth after $n$ patient pairs (i.e., $n$ outcomes per arm) is
...
Beyond the betting construction (which we use for binary endpoints), two other approaches are useful in clinical settings [@RamdasGrunwaldVovkShafer2023SAVI; @GrunwaldDeHeideKoolen2023SafeTesting]:
...
E-value proponents face an analogous translation challenge to the one Berry navigated over three decades. Practical communication strategies include presenting the e-value monitoring plan as a pre-planned sequential analysis method with a single time-uniform threshold, providing always-valid $p$-values and confidence sequences alongside the e-process for familiar reporting, and including simulation-based operating characteristics (power, expected sample size, Type I error) in the statistical analysis plan, as is standard for group sequential and Bayesian designs. The FDA's Complex Innovative Design (CID) initiative encourages novel trial designs for demonstrating effectiveness [@fda2019adaptive], and the 2026 draft Bayesian guidance's openness to non-traditional success criteria may together provide a natural regulatory pathway for e-value-based monitoring in confirmatory trials.
...
A conservative choice ($\lambda = 0.10$) yields almost no power, because the per-observation growth rate is too slow to overcome the rejection threshold in `r Nmax` observations. The GROW-optimal $\lambda \approx `r round(lambda_star, 2)`$ maximizes power. Aggressive choices ($\lambda \ge 0.40$) sacrifice power because the increased variance of the log-e-process outweighs the higher expected drift. In all cases, Type I error remains well below $\alpha$, confirming the robustness of anytime-valid control to the betting fraction. This sensitivity analysis underscores the importance of calibrating $\lambda$ to the design alternative, in the same spirit that group sequential designs calibrate the spending function and Bayesian designs calibrate the prior.
...
The numerical study on a two-arm binary trial provides concrete quantification: among error-controlling methods, the calibrated GS boundary achieves the highest power (86.1%) under a fixed 20-look schedule, the calibrated Bayesian rule is more conservative (68.8%), and the e-value provides robust anytime-valid control (72.3%). Crucially, the extended simulations show this ranking reverses under continuous monitoring (e-value 75%, GS 10%), and the power gap varies from 2 to 29 percentage points depending on the effect size and base rate. The futility demonstration confirms that e-processes provide effective futility monitoring, and the RECOVERY-like simulation shows the methodology scales to large trials while highlighting the sample-size cost of small effects. These findings motivate hybrid approaches in which e-values complement, rather than replace, group sequential and Bayesian methods.

For practitioners, a concise recommendation is as follows. E-values are most appropriate when a trial will realistically face flexible monitoring or data-dependent continuation and adaptation, and when an auditable validity guarantee under those operations is a first-order requirement---precisely the settings that Berry's platform trials (I-SPY 2, GBM AGILE, REMAP-CAP) have shown to be increasingly common [@berry2025adaptive]. Conventional group sequential designs are preferable when the monitoring plan is stable and regulatory-facing power efficiency under standard assumptions is paramount. Bayesian predictive probabilities, as Berry has demonstrated across numerous landmark trials, are natural when the operational decision is explicitly about the probability of future trial success and when algorithmic adaptation should drive enrollment and allocation in real time; in such designs, thresholds should be calibrated for frequentist operating characteristics when such guarantees are required. The most promising path forward is not to choose one framework over the others but to combine them: Bayesian predictive-probability algorithms for operational decision-making, group sequential boundaries for power-optimized confirmatory analysis at pre-planned looks, and e-processes as an anytime-valid evidential ledger that remains valid regardless of how many times the data are inspected, how the schedule changes, or whether the trial continues beyond its planned horizon.

The regulatory landscape favors such integration. The 2026 FDA draft Bayesian guidance [@fda2026bayesian_drugs] opens three pathways for success criteria---Type I error calibration, direct posterior interpretation, and decision-theoretic approaches---each of which can accommodate e-value monitoring. E-values are not merely a new test statistic; they are a practical methodology for aligning real-world trial conduct with defensible error control, in a regulatory environment that is now formally prepared to evaluate such innovation.

The `evalinger` R package (\url{https://github.com/VadimSokolov/evalinger}) and its interactive web application (\url{https://sailtargets.shinyapps.io/evalinger/}) are designed to make this methodology accessible to the clinical trial community; they complement the broader treatment of adaptive trial methodology in @SokolovaSokolov2026ClinicalTrials. By providing a complete implementation---from GROW-optimal design calibration through real-time monitoring to platform trial multiplicity control---alongside a head-to-head comparison engine that reproduces the numerical study in this paper, the toolkit enables practitioners to evaluate e-values against group sequential and Bayesian alternatives under their own trial parameters. The web application's three dashboards (Design Calculator, Monitoring Dashboard, and Method Comparison) are specifically structured to generate the operating characteristics documentation that the 2026 FDA draft guidance requires [@fda2026bayesian_drugs]: Type I error rates, power curves, expected sample sizes under design priors, and sensitivity to calibration parameters. As the regulatory framework continues to evolve---through the finalization of the Bayesian guidance, the ongoing ICH E20 initiative on adaptive designs, and the FDA's Complex Innovative Design program---we intend for `evalinger` to serve as a living implementation that tracks methodological advances and regulatory expectations, lowering the barrier for statisticians and clinical operations teams to adopt anytime-valid monitoring in the adaptive trials that the field increasingly demands.
